...
class FavoritesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.favoritesTracksRecyclerView.layoutManager = LinearLayoutManager(requireContext(),LinearLayoutManager.VERTICAL, false)
        binding.favoritesTracksRecyclerView.adapter = adapterFavorite

        viewModel.getStateFavoritesTracks().observe(viewLifecycleOwner){
            adapterFavorite?.trackList?.clear()
            it.listFavoritesTracks?.let { list -> adapterFavorite?.trackList?.addAll(list) }
            adapterFavorite?.notifyDataSetChanged()

            binding.apply {
                favoritesTracksRecyclerView.isVisible = it.isVisibleFavorites
                placeholderNoFavorites.isVisible = it.isVisiblePlaseholder
            }
